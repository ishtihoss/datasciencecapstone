...
RMSE(validation$rating,predicted_ratings_1)

```
We see that after adding user effect and movie effect, the RMSE is improved considerably. Now we are going to take the genre effect into account.

```{r,include=TRUE,echo=TRUE}

genre_avgs <- edx_genre_split %>% 
  left_join(movie_avgs, by='movieId') %>% left_join(user_avgs,by='userId') %>%
  group_by(genres) %>%
  summarize(b_g = mean(rating - mu - b_i-b_u))

predicted_ratings_2 <- validation %>% separate_rows(genres, sep ="\\|") %>%
  left_join(movie_avgs, by='movieId') %>%
  left_join(user_avgs, by='userId') %>%
...
RMSE(validation$rating,predicted_ratings_2)
